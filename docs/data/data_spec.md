...
For MVP, the approved acquisition layer is the user's TradingView <-> Claude MCP bridge.

This means:
- the bridge may be used to pull raw historical candles
- the bridge may be used to confirm symbol and timeframe availability
- the bridge may be used to export datasets for normalization

However, official experiments must use:
- saved raw extracts
- normalized processed datasets
- versioned dataset identifiers
- logged extraction metadata

### Official MVP acquisition definition
- Official acquisition layer: `TradingView MCP bridge`
- Official chart source symbol: `COINBASE:BTCUSD`
...
Raw MCP exports must be stored under:

- `data/raw/tradingview_mcp/<symbol>/<timeframe>/`
...
Raw MCP extraction datasets must use this format:

- `tvmcp_<symbol>_<timeframe>_<timezone>_<pull-date>`
...
Examples:
- `tvmcp_COINBASE_BTCUSD_1D_UTC_2026-03-03`
- `tvmcp_COINBASE_BTCUSD_4H_UTC_2026-03-03`
- `proc_COINBASE_BTCUSD_1D_UTC_2026-03-03_v1`
- `proc_COINBASE_BTCUSD_4H_UTC_2026-03-03_v1`
...
- market: BTC/USD
- TradingView symbol: `COINBASE:BTCUSD`
...
- TradingView role: acquisition + visual validation
